...
public class RestDataGroupToCoraConverter implements RestToCoraConverter {
	protected RestDataGroup restDataGroup;
	protected DataGroup dataGroup;

	public static RestToCoraConverter fromRestDataGroup(RestDataGroup restDataGroup) {
		return new RestDataGroupToCoraConverter(restDataGroup);
	}

	protected RestDataGroupToCoraConverter(RestDataGroup restDataGroup) {
	...
	private void addRecordLinkChild(RestDataElement restDataElement) {
		DataRecordLink dataGroupChild = (DataRecordLink) RestDataRecordLinkToCoraConverter
	...
	private void addResourceLinkChild(RestDataElement restDataElement) {
		DataResourceLink dataResourceLink = (DataResourceLink) RestDataResourceLinkToCoraConverter
				.fromRestDataGroup((RestDataGroup) restDataElement).convert();
		dataGroup.addChild(dataResourceLink);
	}

	private void addGroupChild(RestDataElement restDataElement) {
		DataGroup dataGroupChild = RestDataGroupToCoraConverter
				.fromRestDataGroup((RestDataGroup) restDataElement).convert();
		dataGroup.addChild(dataGroupChild);
	}

	private void addAtomicChild(RestDataElement restDataElement) {
		DataAtomic dataAtomic = RestDataAtomicToCoraConverter
